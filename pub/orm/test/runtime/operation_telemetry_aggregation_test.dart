...
void main() {
  ...
  group('operation telemetry aggregation', () {
    test('aggregates createMany into one operation record', () async {
      ...
      expect(telemetry?.steps.map((step) => step.trace.phase).toList(), <String>[
        'item.create',
        'item.create',
      ]);
    ...
    test('aggregates fallback update reload into one operation record', () async {
    ...
    test('aggregates fallback delete preload into one operation record', () async {
      final noReturningContract = OrmContract(
        version: contract.version,
        hash: contract.hash,
        models: contract.models,
